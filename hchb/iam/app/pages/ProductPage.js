var ProductPage = Ui({
  init:function($this) {
    ...
    this.skutable = ProductPage.SkuTable($('#skutable'), $('#a_newsku'))
      .on('toggle', this, this.sku_ontoggle)
      .on('save', this, this.sku_onsave);
  ...
  sku_ontoggle:function(editing) {
    this.tabbar.enable(! editing);
    this.feedtable.enable(! editing);
  },
  sku_onsave:function(o) {
    EditProduct.saveSku(this, o, function(rec) {
      this.load(rec);
    })
  },
  ...
  title:function() {
    var title = 'Product : ' + (this.rec ? this.rec.name() : 'New Record');
  ...
  load:function(rec, tab) {
    ...
    this.skutable.load(this.id, rec.skus);
  ...
  SkuTable:Ui({
    ...
    load:function(ipid, recs) {
      var NewRec = Sku.asNew.curry(ipid);
      this.entry.load(NewRec, recs, function(rec) {
        return {row:[rec.value, rec.description, rec.activeText()],active:rec.active};
      })
    },
    enable:function(b) {
      this.entry.enable(b);
  ...
  FeedTable:Ui({
    ...
    enable:function(b) {
      this.enabled = b;
      this.table.$this.toggleClass('inactive', ! b);
      $('#a_new').toggleClass('inactive', ! b);
    }    
  }),
  ConfTable:Ui({
    init:function($this) {
