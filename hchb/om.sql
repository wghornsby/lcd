...
BEGIN
	...
	BEGIN TRY
		DECLARE @VendorID INT = 70
			...
			,@LookbackDaysForApprovedOrder INT
		...
		-- Check for Lookback Days for approved order, Setting default to 10 in case if no or incorrect data found - Ref VSTS 355752
		SELECT @LookbackDaysForApprovedOrder = TRY_CONVERT(INT,RTRIM(LTRIM(vd_value)))
		FROM dbo.VENDOR_DETAILS
		WHERE vd_vid = @VendorID
				AND vd_key = 'OrderApprovedDays'
		IF(@LookbackDaysForApprovedOrder IS NULL OR @LookbackDaysForApprovedOrder < 0 OR @LookbackDaysForApprovedOrder ='')
		BEGIN 
			SET @LookbackDaysForApprovedOrder=10
		END
		...
		BEGIN
			...
			*/
			...
			AND (co.o_dateapproved >= DATEADD(DAY,-@LookbackDaysForApprovedOrder,@StartDate)
		    AND co.o_dateapproved <= @EndDate )
