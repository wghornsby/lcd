  ...
  for (var i = 0; i < sources.length; i++) { 
    if (sources[i] instanceof Object) {
      for (var name in sources[i]) {
        if (sources[i].hasOwnProperty(name) || undef(target[name]) {
...
/* Array */
ec(Array, {
  findValue:/*i*/function(value) {
    return Array.findValue(this, value);
  },
  has:/*b*/function(value) {
    return Array.findValue(this, value) != -1;
  }
},{
  ...
  extend:function(/*proto,..*/) {
    ...
    for (var i = 0; i < protos.length; i++) {
      for (var name in protos[i]) {
        ...
        } else if (protos[i].hasOwnProperty(name) || undef(constructor._proto[name]) {
